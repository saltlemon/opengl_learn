...
void main()
{
	...
	if(cosTheta>spotLight.cosOutPhi){
		...
		diff = max(dot(n, -lightDir), 0.0)* texture(material.diffTex, texCoords).rgb
		* material.diffWeight * spotLight.lightColor*intensity;
	
		vec3 view=normalize(worldPos-camPos);
		vec3 halfV=-normalize(view+lightDir);
		spec = pow(max(dot(n,halfV), 0.0), material.shininess) 
		* texture(material.specTex, texCoords).rgb * material.specWeight * spotLight.lightColor*intensity;
